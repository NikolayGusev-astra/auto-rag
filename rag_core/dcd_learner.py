...
def read_cache() -> list[dict]:
    ...
    В main rag_async._CACHE хранит result-словари, а DCD-домен лежит
    внутри result['_trace'] (JSON-строка RagTrace). Парсим аккуратно.
    """
    ...
    for key, value in list(_CACHE.items()):
        dcd_domain = ""
        dcd_collection = ""
        trace_val = value.get("_trace", "")
        if isinstance(trace_val, str) and trace_val.strip().startswith("{"):
            try:
                td = json.loads(trace_val)
                dcd_domain = td.get("domain", "")
                dcd_collection = td.get("collection", "")
            except Exception:
                pass
        elif isinstance(trace_val, dict):
            dcd_domain = trace_val.get("domain", "")
            dcd_collection = trace_val.get("collection", "")
...
def read_routing_log() -> list[dict]:
    ...
    В main rag_async._log_routing пишет поле 'actual_source'; нормализуем
    его в 'source' для совместимости с остальной логикой learner-а.
    """
            ...
            for line in f:
                if line.strip():
                    try:
                        ...
                        continue
                    if "source" not in rec and "actual_source" in rec:
                        rec["source"] = rec["actual_source"]
                    records.append(rec)
    return records

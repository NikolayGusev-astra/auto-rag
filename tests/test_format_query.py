...
class TestFormatQuery:
    @pytest.fixture
    def client(self):
        return AsyncMCPClient(session=None)
    ...
    def test_benign_query(self, client, jira_template):
        result = client._format_query(jira_template, "postgresql config", 5)
    ...
    def test_jql_injection_attempt(self, client, jira_template):
        ...
        result = client._format_query(jira_template, malicious, 5)
    ...
    def test_single_word_query(self, client, jira_template):
        result = client._format_query(jira_template, "postgresql", 5)
        assert 'text~"postgresql"' in result

    def test_confluence_template(self, client):
        template = '/rest/api/content/search?cql=text~"{query_first3}"&limit={max}'
        result = client._format_query(template, "nginx reverse proxy ssl", 5)
